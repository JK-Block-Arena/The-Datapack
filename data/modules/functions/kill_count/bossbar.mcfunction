...
execute if score #before mem matches ..5399 as @r[scores={state=1..2,team=1..4}] as @s[tag=murder] run scoreboard players add #after mem 50
execute if score #before mem matches ..5399 as @r[scores={state=1..2,team=1..4}] as @s[tag=murder] run scoreboard players add #after mem 50
execute unless score $mutation mem matches 1 if score #before mem matches ..5399 as @r[scores={state=1..2,team=1..4}] as @s[tag=murder] run scoreboard players add #after mem 50
execute unless score $mutation mem matches 1 if score #before mem matches ..5399 as @r[scores={state=1..2,team=1..4}] as @s[tag=murder] run scoreboard players add #after mem 50
# 高于或低于预期人数的微调
execute store result score #people mem if entity @a[scores={state=1..2,team=1..4}]
execute if score #people mem matches ..7 if score #before mem matches ..5399 as @r[scores={state=1..2,team=1..4}] as @s[tag=murder] run scoreboard players add #after mem 50
execute if score #people mem matches ..3 if score #before mem matches ..5399 as @r[scores={state=1..2,team=1..4}] as @s[tag=murder] run scoreboard players add #after mem 50
