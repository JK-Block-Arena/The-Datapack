## 硬直木头
# 检查动没动
execute as @a[tag=9_select_player] if score @s player_id = @e[tag=9_wood_marker,limit=1,sort=nearest,distance=...3] player_id run tag @s add 9_select_success
execute as @a[tag=9_select_player] if score @s player_id = @e[tag=9_wood_marker,limit=1,sort=nearest,distance=.301..] player_id run tag @s add 9_select_fail
