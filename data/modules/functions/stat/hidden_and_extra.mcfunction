scoreboard objectives add dive_5 dummy "折跃灵泉跳水次数"
scoreboard objectives add select_gain_9 dummy "你说我做中成功应对次数"

# 玩家隐秘及彩蛋成就
scoreboard objectives add hidden_campfire dummy "隐秘-希望之火"
scoreboard objectives add hidden_spike dummy "隐秘-揍我"
scoreboard objectives add hidden_star dummy "隐秘-星光绽放"
scoreboard objectives add hidden_arrow dummy "隐秘-一步到位"
scoreboard objectives add hidden_border dummy "隐秘-探索之旅"
scoreboard objectives add hidden_room dummy "隐秘-寻宝"
scoreboard objectives add hidden_diver dummy "隐秘-跳水运动员"
scoreboard objectives add hidden_answer dummy "隐秘-一口气全做对"

scoreboard objectives add extra_first dummy "彩蛋-第一个彩蛋"
scoreboard objectives add extra_sky dummy "彩蛋-卡门线"
scoreboard objectives add extra_bed dummy "彩蛋-睡觉"
scoreboard objectives add extra_believer dummy "彩蛋-信徒"
scoreboard objectives add extra_bell dummy "彩蛋-丧钟为谁而鸣"
scoreboard objectives add extra_2players_1arrow dummy "彩蛋-一箭双雕"

scoreboard objectives add extra_rick_head dummy "彩蛋-rick头"
scoreboard objectives add extra_rick_hand dummy "彩蛋-rick手"
scoreboard objectives add extra_rick_foot dummy "彩蛋-rick脚"
scoreboard objectives add extra_rick_body dummy "彩蛋-rick身"
scoreboard objectives add rick_tp trigger "彩蛋-rick解锁"

scoreboard objectives add match_1_V2_1st dummy "友谊赛-1-v2第一"
scoreboard objectives add match_1_V2_2nd dummy "友谊赛-1-v2第二"
scoreboard objectives add match_1_V2_3rd dummy "友谊赛-1-v2第三"

scoreboard objectives add match_2_V1_1st dummy "友谊赛-2-v1第一"
scoreboard objectives add match_2_V1_2nd dummy "友谊赛-2-v1第二"
scoreboard objectives add match_2_V1_3rd dummy "友谊赛-2-v1第三"

scoreboard objectives add match_2_V2_1st dummy "友谊赛-2-v2第一"
scoreboard objectives add match_2_V2_2nd dummy "友谊赛-2-v2第二"
scoreboard objectives add match_2_V2_3rd dummy "友谊赛-2-v2第三"

scoreboard objectives add match_2_V3_1st dummy "友谊赛-2-v3第一"
scoreboard objectives add match_2_V3_2nd dummy "友谊赛-2-v3第二"
scoreboard objectives add match_2_V3_3rd dummy "友谊赛-2-v3第三"

# pck = PlayerClassKill
scoreboard objectives add pck1 dummy "初窥门径计数"
scoreboard objectives add pck2 dummy "日益精进计数"
scoreboard objectives add pck3 dummy "驾轻就熟计数"
scoreboard objectives add pck4 dummy "炉火纯青计数"
scoreboard objectives add pck5 dummy "登峰造极计数"
scoreboard objectives add pck6 dummy "出神入化计数"
scoreboard objectives add pck_max dummy "返璞归真计数"

scoreboard objectives add particle_rainbow dummy "彩虹粒子解锁"