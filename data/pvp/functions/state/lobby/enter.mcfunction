...
# 重置物品栏与队伍
team join lobby @s
execute unless score #match_mode mem matches 1 as @s[scores={leave_game=1..,team=1..6}] run function pvp:state/lobby/enter_from_arena
execute if score #match_mode mem matches 1 run tellraw @s {"text": "\n※  正在进行友谊赛，你是参赛玩家还是观众呢？\n","color": "gold"}
