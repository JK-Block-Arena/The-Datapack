...
# 卡门线
#execute as @s[x=-2077,y=128,z=-1689,dx=2,dy=0,dz=2,scores={tip_0_tutorial=7..,state=0}] run effect give @s levitation 2 120
...
# 丧钟为谁而鸣
execute unless block 8039 63 8081 target[power=0] run scoreboard players set @a[gamemode=adventure,x=8039,y=63,z=8081,distance=..30] extra_bell 1
# rick头
...
# 友谊赛自定义称号
execute unless score #match_mode mem matches 1 as @s[team=lobby,scores={state=0},advancements={pvp:match/1/v2_1st=true}] run team join match_1_v2_1st

execute unless score #match_mode mem matches 1 as @s[team=lobby,scores={state=0},advancements={pvp:match/2/v1_1st=true}] run team join match_2_v1_1st
execute unless score #match_mode mem matches 1 as @s[team=lobby,scores={state=0},advancements={pvp:match/2/v2_1st=true}] run team join match_2_v2_1st
execute unless score #match_mode mem matches 1 as @s[team=lobby,scores={state=0},advancements={pvp:match/2/v3_1st=true}] run team join match_2_v3_1st
